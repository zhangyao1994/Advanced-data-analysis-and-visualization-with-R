...
# iris.1to6 <- iris[1:6,]
iris.1to6 <- iris[140:150,]
iris.repeated1to6 <- iris.1to6[rep(seq_len(nrow(iris.1to6)), each=6),]
iris.repeated1to6

iris.repeated <- rbind(iris.repeated1to6,iris)
```
...
# iris.repeated
iris.repeated %>% select(-Species) %>% # remove Species column
  scale() %>%                 # scale to 0 mean and unit variance
  prcomp() ->                 # do PCA
...
pca_data1 <- data.frame(pca1$x, Species=iris.repeated$Species)
head(pca_data1)# iris.repeated
```
Now we can plot as usual:
```{r}
ggplot(pca_data, aes(x=PC1, y=PC2, color=Species)) + geom_point()
ggplot(pca_data1, aes(x=PC1, y=PC2, color=Species)) + geom_point()# iris.repeated
```
