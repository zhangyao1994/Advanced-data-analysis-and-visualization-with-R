...
pca_data1 <- data.frame(pca1$x, Species=iris$Species)
head(pca_data1)# no scale
```
Now we can plot as usual:
```{r}
ggplot(pca_data, aes(x=PC1, y=PC2, color=Species)) + geom_point() # scale
ggplot(pca_data1, aes(x=PC1, y=PC2, color=Species)) + geom_point()# no scale
```
