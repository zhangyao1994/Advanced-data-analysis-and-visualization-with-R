...
```{r}
biopsy %>% select(-outcome) %>% # remove outcome column
  scale() %>%                 # scale to 0 mean and unit variance
  prcomp() ->                 # do PCA
  pca                         # store result as `pca`
...
ggplot(pca_data, aes(x=PC1, y=PC3, color=outcome)) + geom_point()
ggplot(pca_data, aes(x=PC2, y=PC3, color=outcome)) + geom_point() # bad
```
